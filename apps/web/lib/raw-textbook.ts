...
export const getRawTextbookChapters = cache(async (textbookFile: string): Promise<RawChapter[]> => {
...
export const createPartialTextbookChapter = (chapter: RawChapter, sectionId: string): string => {
	const partialSections = chapter.sections.filter((section) => section.id <= sectionId);
	const sectionsText = partialSections
		.map((section) => `## ${chapter.id}.${section.id}: ${section.title}\n${section.content}`)
		.join('\n');
