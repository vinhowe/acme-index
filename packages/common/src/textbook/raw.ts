...
export const parseRawTextbookChapters = async (
  textbookContent: string,
): Promise<Record<string, RawChapter>> => {
  const chapters = parseRawTextbook(textbookContent);
...
export const createPartialTextbookChapter = (
  ...
  const partialSections = chapter.sections.filter(
    (section) => parseInt(section.id) <= sectionNumber,
  );
